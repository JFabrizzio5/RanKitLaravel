...
const gameNeon   = computed(() => props.game === 'valorant' ? '#f43f5e' : '#f59e0b')
...
<template>
  ...
  <div class="min-h-screen bg-[#050505] text-white pb-20 overflow-x-hidden"
    ...
    <!-- Top bar -->
    <div class="fixed top-0 left-0 w-full z-50 flex items-center justify-between px-6 lg:px-16 h-14 border-b border-white/5 bg-[#050505]/90 backdrop-blur-md">
      <div class="flex items-center gap-4">
        <Link href="/game-selector" class="flex items-center gap-1 text-xs font-bold uppercase tracking-wider text-gray-400 hover:text-white transition">
          <span>←</span> Juegos
        </Link>
        ...
        >
          ...
          <div class="p-5 space-y-3">
            <div class="flex items-start justify-between gap-2">
              <h3 class="font-black text-base uppercase leading-tight tracking-tight flex-1" style="font-family: 'Chakra Petch', sans-serif">
                {{ t.name }}
              </h3>
              <button
            ...
            <p class="text-[10px] text-gray-600 font-mono">
              {{ t.format === 'swiss_elimination' ? `${t.swiss_rounds_total} rondas Swiss → Top ${t.elimination_teams}` : `Bracket desde inicio` }}
            </p>

            <Link
              :href="route('lol.show', t.id)"
    ...
    <Teleport to="body">
      <Transition name="fade">
        <div v-if="showCreate" class="fixed inset-0 z-[100] flex items-center justify-center p-4"
          ...
          <div class="relative z-10 w-full max-w-md bg-[#0e0e0e] border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
            <!-- Header -->
            <div class="flex items-center justify-between px-6 py-4 border-b border-white/5">
              <h2 class="font-black uppercase text-lg tracking-tight" style="font-family: 'Chakra Petch', sans-serif">
                Nuevo torneo
              </h2>
              <button @click="showCreate = false" class="text-gray-500 hover:text-white transition text-xl leading-none">×</button>
            </div>
            ...
            <form @submit.prevent="createTournament" class="p-6 space-y-5">
              ...
              <!-- Nombre -->
              <div>
                <label class="block text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-1.5">Nombre del torneo</label>
                <input
                  v-model="form.name"
                  type="text"
              ...
              <!-- Formato -->
              <div>
                <label class="block text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-2">Formato del torneo</label>
                <div class="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    @click="form.format = 'swiss_elimination'"
                    class="p-3 rounded-lg border text-xs font-bold uppercase text-center transition-all"
                    :class="form.format === 'swiss_elimination'
                  ...
                  <button
                    type="button"
                    @click="form.format = 'elimination'"
                    class="p-3 rounded-lg border text-xs font-bold uppercase text-center transition-all"
                    :class="form.format === 'elimination'
                      ? 'border-yellow-500 bg-yellow-500/10 text-yellow-400'
                      : 'border-white/10 text-gray-500 hover:border-white/30 hover:text-gray-300'"
                  >
              ...
              <div v-if="form.format === 'swiss_elimination'" class="grid grid-cols-2 gap-3">
                <div>
                  <label class="block text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-1.5">Rondas Swiss</label>
                  <input
                    v-model.number="form.swiss_rounds_total"
                ...
                <div>
                  <label class="block text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-1.5">Top equipos → Elim.</label>
                  <input
                    v-model.number="form.elimination_teams"
