...
// --- Computed ---
const gameNeon   = computed(() => props.tournament.game === 'valorant' ? '#f43f5e' : '#f59e0b')
...
function copyWidget() {
  ...
  setTimeout(() => { widgetCopied.value = false }, 2000)
}
...
function labelRound(phase: string, round: number) {
  ...
  if (total - pos === 1) return 'Gran Final'
  if (total - pos === 2) return 'Semifinal'
  ...
  <div class="min-h-screen bg-[#050505] text-white pb-20 overflow-x-hidden" style="font-family:'Archivo',sans-serif;">
    ...
    <div class="fixed top-0 left-0 w-full z-50 flex items-center justify-between px-5 lg:px-12 h-14 border-b border-white/5 bg-[#050505]/90 backdrop-blur-md">
      <div class="flex items-center gap-3">
        <Link :href="`/lol?game=${tournament.game}`" class="text-xs font-bold uppercase tracking-wider text-gray-400 hover:text-white transition">← Torneos</Link>
        <div class="w-px h-4 bg-white/10"></div>
        <span class="text-xs font-bold uppercase tracking-widest" :style="{ color: gameNeon }">{{ gameIcon }} {{ tournament.name }}</span>
      </div>
      <div class="flex items-center gap-3">
        <!-- Widget OBS button -->
        <button @click="copyWidget"
                class="flex items-center gap-1.5 px-3 py-1 text-[10px] font-bold uppercase rounded border border-white/15 text-gray-400 hover:text-white hover:border-white/40 transition">
          📺 {{ widgetCopied ? '¡Copiado!' : 'Copiar URL Widget OBS' }}
        </button>
        <a :href="widgetUrl" target="_blank"
           class="flex items-center gap-1 px-2 py-1 text-[10px] font-bold uppercase rounded border border-white/10 text-gray-500 hover:text-white transition">
          Vista previa →
        </a>
        <!-- Phase badge -->
    ...
    <!-- Champion banner -->
    <div v-if="champion" class="fixed top-14 left-0 w-full z-40 text-center py-2 text-sm font-black uppercase tracking-widest" :style="{ background: `${gameNeon}20`, color: gameNeon }">
    ...
    <main class="max-w-7xl mx-auto px-5 pt-24 grid grid-cols-1 lg:grid-cols-12 gap-6" :class="{ 'pt-32': !!champion }">
      ...
      <aside class="lg:col-span-4 space-y-4">
        ...
        <div class="bg-[#0c0c0c] border border-white/5 rounded-xl overflow-hidden">
          <div class="flex items-center justify-between px-4 py-3 border-b border-white/5">
            <h2 class="text-xs font-black uppercase tracking-widest" style="font-family:'Chakra Petch',sans-serif">Equipos <span class="text-gray-500">({{ teams.length }})</span></h2>
            <div class="flex gap-1">
              <button @click="shuffle" title="Aleatorio" class="p-1.5 text-gray-500 hover:text-white border border-white/10 rounded text-xs transition">🔀</button>
              <button @click="sortByName" title="A-Z" class="p-1.5 text-gray-500 hover:text-white border border-white/10 rounded text-xs transition">🔤</button>
            </div>
          </div>
          ...
          <div class="px-4 py-3 border-b border-white/5 space-y-2">
            <div class="flex gap-2">
              <input v-model="newTeamName" type="text" placeholder="Nombre del equipo..."
                     class="flex-1 bg-white/5 border border-white/10 rounded px-2 py-1.5 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-yellow-500 transition"
                     @keyup.enter="addTeam" />
              <button @click="addTeam" class="px-3 py-1.5 text-xs font-bold rounded text-black" :style="{ background: gameNeon }">+</button>
            </div>
          ...
          <div class="divide-y divide-white/5 max-h-80 overflow-y-auto">
            <div v-if="teams.length === 0" class="px-4 py-6 text-center text-gray-600 text-xs">Agrega equipos para comenzar</div>
            <div v-for="team in teams" :key="team.id" class="flex items-center justify-between px-4 py-2 hover:bg-white/5 transition group">
              <div class="flex items-center gap-2">
                <span class="text-[10px] font-mono text-gray-600 w-4">{{ team.seed ?? '—' }}</span>
                <!-- Logo -->
                <div class="w-6 h-6 rounded-full overflow-hidden bg-white/5 border border-white/10 flex items-center justify-center text-[9px] font-bold flex-shrink-0" :style="{ color: gameNeon }">
              ...
              <div class="flex items-center gap-2">
                ...
                <button @click="openEdit(team)" class="text-gray-700 hover:text-white transition text-xs opacity-0 group-hover:opacity-100">✎</button>
                <button @click="removeTeam(team.id)" class="text-gray-700 hover:text-red-400 transition text-xs opacity-0 group-hover:opacity-100">✕</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-2">
          ...
          <button v-else-if="tournament.format==='elimination' && tournament.phase==='elimination' && elimMatches.length===0"
                  @click="generate"
                  class="w-full py-2.5 text-xs font-black uppercase rounded border transition-all"
                  :style="{ borderColor: gameNeon, color: gameNeon }"
                  @mouseenter="(e:any)=>e.currentTarget.style.background=`${gameNeon}15`"
          ...
          <button v-if="tournament.format==='swiss_elimination' && allSwissDone && currentSwissRound >= tournament.swiss_rounds_total && tournament.phase==='swiss'"
                  @click="advancePhase"
        ...
        <div v-if="swissMatches.length > 0" class="bg-[#0c0c0c] border border-white/5 rounded-xl overflow-hidden">
          <div class="px-4 py-3 border-b border-white/5">
            <h2 class="text-xs font-black uppercase tracking-widest" style="font-family:'Chakra Petch',sans-serif">Clasificación Swiss</h2>
          </div>
          <div class="divide-y divide-white/5">
            <div v-for="(team,idx) in [...teams].sort((a,b)=>b.wins-a.wins||a.losses-b.losses)" :key="team.id" class="flex items-center gap-3 px-4 py-2">
              <span class="text-[10px] font-mono text-gray-600 w-4">{{ idx+1 }}</span>
              <div class="w-5 h-5 rounded-full overflow-hidden bg-white/5 border border-white/10 flex items-center justify-center text-[8px] font-bold flex-shrink-0" :style="{ color: gameNeon }">
      ...
      <div class="lg:col-span-8 space-y-6">
        ...
        <template v-if="tournament.format==='swiss_elimination' && swissMatches.length>0">
          <div v-for="(roundMatches,round) in swissRounds" :key="round">
            <div class="flex items-center gap-3 mb-3">
              ...
              <h3 class="text-xs font-black uppercase tracking-widest px-3 py-1 rounded border" :style="{ borderColor:`${gameNeon}40`, color:gameNeon }">
                ...
                <div v-else>
                  <div class="flex items-center justify-between gap-2">
                    <div class="flex items-center gap-1.5 flex-1 min-w-0">
                      <div class="w-5 h-5 rounded-full overflow-hidden bg-white/5 flex-shrink-0 flex items-center justify-center text-[8px]" :style="{color:gameNeon}">
                        <img v-if="match.team1?.logo" :src="match.team1.logo" class="w-full h-full object-cover" /><span v-else>{{ match.team1?.name[0] }}</span>
                      </div>
                      <span class="font-bold text-sm truncate" :style="match.winner_id===match.team1_id?{color:gameNeon}:{}">{{ match.team1?.name }}</span>
                    </div>
                    <span class="text-xs text-gray-500 font-bold">VS</span>
                    <div class="flex items-center gap-1.5 flex-1 min-w-0 justify-end">
                      <span class="font-bold text-sm truncate" :style="match.winner_id===match.team2_id?{color:gameNeon}:{}">{{ match.team2?.name }}</span>
                      <div class="w-5 h-5 rounded-full overflow-hidden bg-white/5 flex-shrink-0 flex items-center justify-center text-[8px]" :style="{color:gameNeon}">
                        <img v-if="match.team2?.logo" :src="match.team2.logo" class="w-full h-full object-cover" /><span v-else>{{ match.team2?.name[0] }}</span>
                      </div>
                    </div>
                  </div>
                  <div v-if="match.status==='done'" class="text-center mt-2 text-lg font-black font-mono tracking-widest" :style="{color:gameNeon}">
        ...
        <!-- Empty state -->
        <div v-if="matches.length===0 && teams.length>=2" class="bg-[#0c0c0c] border border-dashed border-white/10 rounded-xl p-12 text-center space-y-3">
          <p class="text-gray-500 text-sm">{{ tournament.format==='swiss_elimination' ? 'Genera la primera ronda Swiss para comenzar.' : 'Genera el bracket de eliminación para comenzar.' }}</p>
          <button @click="generate" class="px-6 py-2 text-xs font-bold uppercase rounded border transition-all" :style="{borderColor:gameNeon,color:gameNeon}">Generar bracket →</button>
        </div>
        <div v-if="matches.length===0 && teams.length<2" class="bg-[#0c0c0c] border border-dashed border-white/10 rounded-xl p-12 text-center">
        ...
        <template v-if="elimMatches.length>0">
          <div class="flex items-center gap-3">
            <div class="h-px flex-1 bg-white/5"></div>
            <h3 class="text-xs font-black uppercase tracking-widest px-3 py-1 rounded border border-blue-500/30 text-blue-400">🔵 Eliminación Directa</h3>
            <div class="h-px flex-1 bg-white/5"></div>
          </div>
          <div v-for="(roundMatches,round) in elimRounds" :key="round" class="space-y-3">
            <p class="text-[10px] font-bold uppercase tracking-widest text-gray-500 text-center">{{ labelRound('elimination',Number(round)) }}</p>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div v-for="match in roundMatches" :key="match.id"
                ...
                <div v-if="!match.team2" class="text-xs text-gray-500 text-center"><span class="font-bold text-white">{{ match.team1?.name }}</span> · BYE ✅</div>
                <div v-else>
                  <div class="flex items-center justify-between gap-2">
                    <div class="flex items-center gap-1.5 flex-1 min-w-0">
                      ...
                      <span class="font-bold text-sm truncate" :style="match.winner_id===match.team1_id?{color:gameNeon}:{}">{{ match.team1?.name ?? 'TBD' }}</span>
                    </div>
                    <span class="text-xs text-gray-500 font-bold">VS</span>
                    <div class="flex items-center gap-1.5 flex-1 min-w-0 justify-end">
                      <span class="font-bold text-sm truncate" :style="match.winner_id===match.team2_id?{color:gameNeon}:{}">{{ match.team2?.name ?? 'TBD' }}</span>
                      <div class="w-5 h-5 rounded-full overflow-hidden bg-white/5 flex-shrink-0 flex items-center justify-center text-[8px] text-blue-400">
                        <img v-if="match.team2?.logo" :src="match.team2.logo" class="w-full h-full object-cover" /><span v-else>{{ match.team2?.name?.[0] }}</span>
                      </div>
                    </div>
                  </div>
                  <div v-if="match.status==='done'" class="text-center mt-2 text-lg font-black font-mono tracking-widest text-blue-400">{{ match.score1 }} – {{ match.score2 }}</div>
                  <button v-if="match.status==='pending'" @click="openResult(match)"
    ...
    <Teleport to="body">
      <Transition name="fade">
        <div v-if="resultModal" class="fixed inset-0 z-[200] flex items-center justify-center p-4" @click.self="resultModal=null">
          <div class="absolute inset-0 bg-black/80 backdrop-blur-sm"></div>
          <div class="relative z-10 w-full max-w-sm bg-[#0e0e0e] border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
            <div class="flex items-center justify-between px-5 py-4 border-b border-white/5">
              <h2 class="font-black uppercase text-base tracking-tight" style="font-family:'Chakra Petch',sans-serif">Resultado</h2>
            ...
            <form @submit.prevent="submitResult" class="p-5 space-y-4">
              <div class="text-center text-sm font-bold">{{ resultModal.team1?.name }} <span class="text-gray-500">vs</span> {{ resultModal.team2?.name }}</div>
              <div>
                ...
                <div class="grid grid-cols-2 gap-2">
                  <button type="button" @click="resultForm.winner_id=resultModal!.team1_id"
                          ...
                          :class="resultForm.winner_id===resultModal.team1_id?'border-yellow-500 bg-yellow-500/10 text-yellow-400':'border-white/10 text-gray-400 hover:border-white/30'">
                    {{ resultModal.team1?.name }}
                  </button>
                  <button type="button" @click="resultForm.winner_id=resultModal!.team2_id!"
                          class="py-3 rounded-lg border text-xs font-bold uppercase transition-all"
                          :class="resultForm.winner_id===resultModal.team2_id?'border-yellow-500 bg-yellow-500/10 text-yellow-400':'border-white/10 text-gray-400 hover:border-white/30'">
              ...
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="block text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-1">Score {{ resultModal.team1?.name }}</label>
                  <input v-model.number="resultForm.score1" type="number" min="0" class="w-full bg-white/5 border border-white/10 rounded px-2 py-1.5 text-xs text-white focus:outline-none focus:border-yellow-500 transition" />
                </div>
                <div>
                  <label class="block text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-1">Score {{ resultModal.team2?.name }}</label>
                  <input v-model.number="resultForm.score2" type="number" min="0" class="w-full bg-white/5 border border-white/10 rounded px-2 py-1.5 text-xs text-white focus:outline-none focus:border-yellow-500 transition" />
                </div>
              </div>
              <div class="flex gap-2 pt-1">
                <button type="button" @click="resultModal=null" class="flex-1 py-2 text-xs font-bold uppercase border border-white/10 text-gray-400 rounded-lg hover:border-white/30 transition">Cancelar</button>
    ...
    <Teleport to="body">
      <Transition name="fade">
        <div v-if="editTeam" class="fixed inset-0 z-[200] flex items-center justify-center p-4" @click.self="editTeam=null">
          <div class="absolute inset-0 bg-black/80 backdrop-blur-sm"></div>
          <div class="relative z-10 w-full max-w-sm bg-[#0e0e0e] border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
            <div class="flex items-center justify-between px-5 py-4 border-b border-white/5">
              <h2 class="font-black uppercase text-base tracking-tight" style="font-family:'Chakra Petch',sans-serif">Editar Equipo</h2>
            ...
            <form @submit.prevent="saveEdit" class="p-5 space-y-4">
              ...
              <div class="flex justify-center">
                <div class="w-16 h-16 rounded-full overflow-hidden bg-white/5 border-2 border-white/10 flex items-center justify-center text-2xl font-black" :style="{color:gameNeon}">
                  <img v-if="editForm.logo" :src="editForm.logo" class="w-full h-full object-cover" onerror="this.style.display='none'" />
              ...
              <div>
                ...
                <input v-model="editForm.name" type="text" required
                       class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-yellow-500 transition" />
              </div>
              <div>
                <label class="block text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-1.5">URL de logo (imagen)</label>
                <input v-model="editForm.logo" type="text" placeholder="https://..."
                       class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-yellow-500 transition" />
              </div>
              <div class="flex gap-2 pt-1">
                <button type="button" @click="editTeam=null" class="flex-1 py-2 text-xs font-bold uppercase border border-white/10 text-gray-400 rounded-lg hover:border-white/30 transition">Cancelar</button>
