...
// --- LOGICA DE TEMA (Copiada del Home) ---
...
<template>
    ...
    <div class="min-h-screen overflow-x-hidden bg-gray-50 text-gray-900 dark:bg-[#050505] dark:text-white font-sans transition-colors duration-300 relative flex flex-col">
        ...
        <div class="relative z-10 flex flex-col items-center justify-center flex-1 px-6 py-24 sm:px-0">
            ...
            <div v-if="status" class="mb-4 text-sm font-medium text-green-600 dark:text-green-400 font-mono">
                {{ status }}
            </div>
            ...
            <div class="w-full max-w-md brutal-card p-8 bg-white dark:bg-[#0a0a0a]">
                <div class="mb-8 text-center">
                    <h2 class="text-3xl font-black text-black uppercase font-display dark:text-white">
                        System <span class="text-neon">Login</span>
                    </h2>
                    <p class="text-xs font-mono text-gray-500 mt-2">INGRESE CREDENCIALES DE ACCESO</p>
                </div>
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="relative flex items-center justify-center w-full my-6 border-t border-gray-300 dark:border-gray-700">
                         <span class="px-3 text-xs font-mono text-gray-500 bg-white dark:bg-[#0a0a0a]">O USE CORREO</span>
                    </div>
                    ...
                    <div class="block mt-4">
                        <label class="flex items-center">
                            <Checkbox name="remember" v-model:checked="form.remember" class="text-neon focus:ring-neon rounded-none border-gray-500 bg-transparent" />
                            <span class="ms-2 text-sm text-gray-600 dark:text-gray-400 font-mono">Recordar sesión</span>
                        </label>
                    </div>
                    ...
                    <div class="flex items-center justify-between mt-8">
                        <Link
                            ...
                            class="text-xs text-gray-500 hover:text-neon transition-colors"
                        >
                            ¿Olvidaste tu contraseña?
                        </Link>

                        <button 
                            class="px-8 py-3 text-sm font-bold tracking-wider uppercase btn-skew"
                            :class="{ 'opacity-25': form.processing }"
                            :disabled="form.processing"
                        >
                            <span class="btn-content">INGRESAR</span>
                        </button>
                    </div>
                </form>
            </div>
            ...
            <div class="mt-8 text-center">
                 <p class="text-sm text-gray-500">
                    ...
                    <Link :href="route('register')" class="text-neon font-bold hover:underline">REGÍSTRATE AQUÍ</Link>
                </p>
            </div>
        </div>
    </div>
...
<style>
/* Estilos extraídos del Home para consistencia */
:root {
  --rankit-neon: #bf00ff;
}
